...
/***/ "./src/_resources/js/_splittype.js":
/*!*****************************************!*\
  !*** ./src/_resources/js/_splittype.js ***!
  \*****************************************/
/*! no static exports found */
...
(function (global, factory) {
  ...
  window.SplitType = function (window, document, undefined) {
    ...
    var _defaults = {
      ...
      position: 'relative',
      absolute: false,
      // alternate syntax for setting position.
      tagName: 'div',
      DEBUG: false
    };
    /***********************
    ...
    function _processElements(elements) {
      var elementsArray = [],
          ...
          len2,
          i,
          k; // A. If elements is a selector string...
    ...
    function _split(element) {
      ...
      splitText = splitLines ? createElement('div') : createFragment(); // 2. Get the element's text content.
      ...
      element.innerHTML = '';
      element.appendChild(splitText); // 5. Add the split words/chars in this element to the array of all split words/chars.

      push.apply(this.words, wordNodes);
      push.apply(this.chars, charNodes); // STOP HERE If not splitting text into lines or using absolute positioning
      ...
      var lines = [],
          ...
          nextsib,
          cs,
          align; // nodes is a live HTML collection of the nodes in this element

      nodes = Data(element).nodes = element.getElementsByTagName(TAG_NAME); // Cache the element's parent and next sibling (for DOM removal).

      parent = element.parentElement;
      nextsib = element.nextElementSibling; // get the computed style object for the element

      cs = window.getComputedStyle(element);
      align = cs.textAlign; // If using absolute position...
      ...
      if (splitLines) {
        ...
        splitText = createFragment(); // 9. Iterate over the arrays in lines (see 11 b)
        ...
        lineNodes = lines.map(function (line) {
          ...
          splitText.appendChild(lineNode = createElement(TAG_NAME, {
          ...
          forEach(line, function (wordNode) {
            ...
            if (splitWords) {
              lineNode.appendChild(wordNode); // ii. If NOT splitting into words...
        ...
        element.replaceChild(splitText, element.firstChild); // 11. Add the split line elements to the array of all split lines
      ...
      if (isAbsolute) {
        ...
        element.style.height = elementHeight + 'px'; // 13. Iterate over all split nodes.
        ...
        forEach(nodes, function (node) {
          ...
          var isLineChild = !isLine && Data(node.parentElement).type === 'line'; // b. Set the top position of the current node.
          ...
          node.style.top = isLineChild ? 0 : Data(node).top + 'px'; // c. Set the left position of the current node.
          ...
          node.style.left = isLine ? contentBox.left + 'px' : (isLineChild ? Data(node).left - contentBox.left : Data(node).left) + 'px'; // d. Set the height of the current node to the cached value.

          node.style.height = Data(node).height + 'px'; // e. Set the width of the current node.
          //    If its a line element, width is equal to the width of the contentBox (see step 9).

          node.style.width = isLine ? contentBox.width + 'px' : Data(node).width + 'px'; // f. Finally, set the node's position to absolute.
      ...
      if (nextsib) parent.insertBefore(element, nextsib);else parent.appendChild(element);
    } // End Function
    ...
    function SplitType(elements, options) {
      ...
      this.isSplit = false; // Merge options with defaults

      this.settings = extend(_defaults, options); // Prepare target elements
    ...
    SplitType.prototype.split = function split(newOptions) {
      ...
      this.revert(); // Create arrays to hold the split lines, words, and characters for this instance.
      ...
      var scrollPos = [window.pageXOffset, window.pageYOffset]; // If new options were passed into the split() method, update settings for the instance.
      ...
      }, this); // Set isSplit to true for the SplitType instance

      this.isSplit = true; // Set scroll position to cached value.

      window.scrollTo.apply(window, scrollPos); // Clear data Cache
...
__webpack_require__(/*! ./_splittype.js */ "./src/_resources/js/_splittype.js");
...
window.youtubePlaylist = function (playlistId) {
  return {
    ...
    fetchPlaylist: function fetchPlaylist() {
      ...
      this.isLoading = true;
      fetch("https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=".concat(this.number, "&playlistId=").concat(playlistId, "&key=").concat(this.apiKey)).then(function (res) {
        return res.json();
      }).then(function (data) {
...
__webpack_require__(/*! /Users/johnpuddephatt/Sites/zerowasteleeds/src/_resources/js/app.js */"./src/_resources/js/app.js");
module.exports = __webpack_require__(/*! /Users/johnpuddephatt/Sites/zerowasteleeds/src/_resources/scss/app.scss */"./src/_resources/scss/app.scss");
