...
module.exports = function(eleventyConfig) {
  ...
  const createMarkup = (path, alt, className, width, height, sizes, resize) => {
    const src = `//res.cloudinary.com/${ eleventyConfig.cloudinary }/image/fetch/${ width ? 'w_' + width : '' }${ height ? ',h_' + height : '' }${(height && width) ? ',c_fill' : ''},q_80/${process.env.URL + path.split(" ").join("%20")}`;
    const srcset = eleventyConfig.srcsetWidths.map(w => {
      return `//res.cloudinary.com/${ eleventyConfig.cloudinary }/image/fetch/${ w ? 'w_' + w : '' }${ (height && width) ? ',h_' + Math.floor(height/width * w) : '' }${(height && width) ? ',c_fill' : ''},q_80/${process.env.URL + path.split(" ").join("%20")} ${w}w`;
    }).join(', ');
